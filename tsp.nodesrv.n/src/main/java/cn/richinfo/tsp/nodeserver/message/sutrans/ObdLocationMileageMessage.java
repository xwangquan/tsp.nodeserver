...
/**
 * @ClassName: ObdLocationMileageMessage
 * @Description: TODO()
 * @author 王全
 * @date 2015-10-10 下午4:36:50
 */
public class ObdLocationMileageMessage extends
	...
	public byte[] dashboardMileage = new byte[4];
	public byte[] obdDashboardMileage = new byte[4];
	public byte[] restOil = new byte[2];
	public byte driveStatus;

	/**
	 * @param ttType
	 */
	public ObdLocationMileageMessage() {
		super(SuTransTypes.OBDLOCATIONMILEAGE);
	}

	// --inner
	private int SUBLEN = 25 + 11;
	...
	public int hei(byte[] receivedData, int offset) throws Exception {
		...
		System.arraycopy(receivedData, offset + 25, dashboardMileage, 0, 4);
		System.arraycopy(receivedData, offset + 29, obdDashboardMileage,
				0, 4);
		System.arraycopy(receivedData, offset + 33, restOil, 0, 2);
		driveStatus = receivedData[offset + 35];
	...
	public byte[] xiu() {
		...
		System.arraycopy(dashboardMileage, 0, send, +25, 4);
		System.arraycopy(obdDashboardMileage, 0, send, +29, 4);
		System.arraycopy(restOil, 0, send, +33, 2);
		send[35] = driveStatus;
		return send;
	}
