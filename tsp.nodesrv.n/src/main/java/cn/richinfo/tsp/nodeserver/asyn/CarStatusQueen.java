...
import cn.richinfo.tsp.nodeserver.persistence.bean.CarStatusBean;
import cn.richinfo.tsp.nodeserver.persistence.info.CarStatus;
import cn.richinfo.tsp.nodeserver.utils.ConversionUtil;

/**
 * @ClassName: {@link CarStatusQueen}
 * @Description: {@link CarStatusMessage} worker
 * @author ÍõÈ«
 * @date 2014-11-20 ÏÂÎç1:50:04
 */
public class CarStatusQueen extends NodeServerQueen<CarStatusMessage> {
	private static Logger log = Logger.getLogger(CarStatusQueen.class);
	private static final long serialVersionUID = -2062036043163585987L;
	private NodeServerContext context;
	private int WORKERS = 1;
	private String HBASE_TABLE = "CarBodyInfo";

	public CarStatusQueen(NodeServerContext contex) {
		this.context = contex;
		init();
	}
		...
		public void run() {
			while (true) {
				try {
					...
					System.out.println("CarStatus size:" + size());
	...
	class HbaseObdLocationWorker extends Thread {
		...
		public void store(CarStatusMessage message) throws Exception {
			CarStatusBean bean = new CarStatusBean();
			...
			CarStatus data = new CarStatus(message);
			bean.setCar_body_info(data.toString());
			...
			put.add("info".getBytes(), Bytes.toBytes("car_position_info"),
					Bytes.toBytes(bean.getCar_body_info() + ""));
		...
		public void run() {
			while (true) {
				try {
					CarStatusMessage message = (CarStatusMessage) poll();
					if (message == null) {
