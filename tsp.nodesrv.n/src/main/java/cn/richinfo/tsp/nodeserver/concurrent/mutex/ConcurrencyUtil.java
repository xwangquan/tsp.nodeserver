...
/**
 * Various bits of black magic garnered from experts on the
 * concurrency-interest@cs.oswego.edu mailing list.
 *
 * @author Greg Luck
 * @version $Id: ConcurrencyUtil.java 889 2009-02-17 09:55:43Z gregluck $
 */
public final class ConcurrencyUtil {
    ...
    /**
     * Returns a hash code for non-null Object x. <p/> This function ensures
     * that hashCodes that differ only by constant multiples at each bit
     * position have a bounded number of collisions. (Doug Lea)
     *
     * @param object
     ...
     */
    public static int selectLock(final Object key, int numberOfLocks) throws Exception {
        int number = numberOfLocks & (numberOfLocks - 1);
        if (number != 0) {
            throw new Exception("Lock number must be a power of two: " + numberOfLocks);
        }
        if (key == null) {
            return 0;
        } else {
